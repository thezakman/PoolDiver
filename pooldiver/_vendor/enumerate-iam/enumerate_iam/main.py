...
def check_one_permission(arg_tuple):
    ...
    try:
        ...
        logger.error(
            "Remove %s.%s action - not found" % (service_name, operation_name),
        )
        return

    logger.debug(
        "Testing %s.%s() in region %s" % (service_name, operation_name, region)
    )
    ...
    try:
        ...
        logger.error(
            "Remove %s.%s action - param validation error"
...
def enumerate_role(iam_client, output):
    ...
    try:
        ...
        for policy in role_policies["AttachedPolicies"]:
            logger.info(
                '-- Policy "%s" (%s)', policy["PolicyName"], policy["PolicyArn"]
            )
...
def enumerate_user(iam_client, output):
    ...
    try:
        ...
        logger.info(
            'User "%s" has %0d groups associated', user_name, len(user_groups["Groups"])
        )
    ...
    for group in user_groups["Groups"]:
        try:
            ...
            output["iam.list_group_policies"][group["GroupName"]] = remove_metadata(
                group_policy
            )
